...
bool FoxAndHounds::move(int rankFrom, int colFrom, int rankTo, int colTo)
{
    if(currentPlayer() == NoPlayer) return false;

    // is there a piece of the right color?
    char source = board()->data(rankFrom, colFrom);
    ...
    char destination = board()->data(rankTo, colTo);
    if(destination != ' ') return false;

    // is white advancing?
    if(currentPlayer() == Player1 && rankTo <= rankFrom) return false;

    board()->movePiece(rankFrom, colFrom, rankTo, colTo); // make the move
...
{
    if(emptyByOffset(-1, -1) || emptyByOffset(-1, 1) || emptyByOffset(1, -1)
            || emptyByOffset(1, 1)) return true;
    return false;
}
...
bool FoxAndHounds::emptyByOffset(int x, int y) const
{
    ...
    if(destCol < 1 || destRank < 1 || destCol > board()->columns()
